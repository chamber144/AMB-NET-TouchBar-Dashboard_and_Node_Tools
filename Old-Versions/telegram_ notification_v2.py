...
dailybundlewarnings = ['60','100','200','300','400','500','1000','5000']
...
api_url_base = 'https://explorer-api.ambrosus.io/atlases'
count = 0
stateatlas = []
statusatlas = []
bundlesatlas = []
...
for each in atlasnodes:
    ...
    while search == 1:
        ...
        if (len(next)) < 2:
            ...
            newlink = ("https://explorer-api.ambrosus.io/atlases?next="+pageEnd)
    ...
    if statusatlas[count] != "ONLINE" and statsat[count] == "1":
        ...
        #send_message('Your Atlas Node '+str(count+1)+' has just gone offline! https://explorer.ambrosus.io/atlas/'+each)
    if statusatlas[count] == "ONLINE" and statsat[count] == "0":
        ...
        #send_message('Your Atlas Node '+str(count+1)+' is back online! <a href="https://explorer.ambrosus.io/atlas/'+each+">"+each+"</a>)
    if feebalance <= 10:
...
api_url_base = 'https://explorer-api.ambrosus.io/apollos'
count = 0
stateapollo = []
statusapollo = []
balanceapollo = []
...
for each in apollonodes:
    ...
    while search == 1:
        ...
        if (len(next)) < 2:
            ...
            pageEnd = pageCloserEnd[1]
            pageEnd = (pageEnd.replace("'", ""))
            newlink = ("https://explorer-api.ambrosus.io/apollos?next="+pageEnd)
            response = requests.get(newlink)
            data = str(response.json())
            found = (data.split(each))
            search = (len(found))
    ...
    if statusapollo[count] != "ONLINE" and statsap[count] == "1":
        ...
        #send_message('Your Apollo Node '+str(count+1)+' has just gone offline! https://explorer.ambrosus.io/apollo/'+each)
    if statusapollo[count] == "ONLINE" and statsap[count] == "0":
        ...
        #send_message('Your Apollo Node '+str(count+1)+' is back online! https://explorer.ambrosus.io/apollo/'+each)
