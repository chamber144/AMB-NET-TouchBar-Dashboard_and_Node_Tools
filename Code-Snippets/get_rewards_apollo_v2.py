...
count = 0
combined = 0.0
csvfile = open(home+"rewards_eur.csv","a")
for each in apollonodes:
    ...
    csv = ("\"Staking\",\""+str(baldif)+"\",\"AMB\",\"\",\"\",\"\",\"\",\"AMB Staking Rewards\",\""+transfer+"\",\"Apollo Node"+str(count+1)+"\",\""+datewrite+"\",\""+priceusd+"\"\n")
    csvfile.writelines(csv)
    combined = combined + baldif
    count = count + 1
csvfile.close()
